...
function resolveShortcuts(name, staticHost, shortcuts) {
...
function parseOne(config, datafile) {
    ...
    data.scripts = data.scripts.map(name => {
        return resolveShortcuts(name, config.hosts.static, REMOTE_SCRIPT_SHORTCUTS);
    });
    data.styles = data.styles.map(name => {
        return resolveShortcuts(name, config.hosts.static, REMOTE_STYLE_SHORTCUTS);
    });
